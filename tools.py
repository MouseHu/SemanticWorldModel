...
class Module(tf.Module):
  ...
  def save(self, filename):#存variable
    values = tf.nest.map_structure(lambda x: x.numpy(), self.variables)#处理嵌套structure
...
def simulate(agent, envs, steps=0, episodes=0, state=None):
  ...
  while (steps and step < steps) or (episodes and episode < episodes):
    ...
    if done.any():
      indices = [index for index, d in enumerate(done) if d]#NOTE:有几个done
      promises = [envs[i].reset(blocking=False) for i in indices]
      for index, promise in zip(indices, promises):
        obs[index] = promise()
    # Step agents.
    obs = {k: np.stack([o[k] for o in obs]) for k in obs[0]}
    # stack all obs to one dict
    # obs: dict 'orientations', 'height', 'velocity', 'image', 'reward'
    ...
    #TODO: Envs should return description in obs.
...
def count_episodes(directory):
  filenames = directory.glob('*.npz')#获取特定后缀文件
  lengths = [int(n.stem.rsplit('-', 1)[-1]) - 1 for n in filenames]#文件名分割
  episodes, steps = len(lengths), sum(lengths)#获取总step和episode长度
  return episodes, steps


def save_episodes(directory, episodes):
  #NOTE:should not be changed since "desc" is saved in episodes.
...
def load_episodes(directory, rescan: int, length=None, balance=False, seed=0):
  '''
  NOTE:should not be changed since descs are in episode.items()
  :param length: randomly draw a sublist of length
  :param balance: change the possibilities by which the sublists are draw.
  '''
  directory = pathlib.Path(directory).expanduser()#展开用户目录
  ...
  while True:
    ...
    for index in random.choice(len(keys), rescan):
      ...
      if length:
        ...
        #FIXME:why SKIP? 
        if available < 1:
          #print(length)config.batch_length
          #print(f'Skipped short episode of length {available}.')
          continue
        else:
          pass
          #many episodes not skipped
          #print(f'not skipped of length {total}')
        if balance:
...
def static_scan(fn, inputs, start, reverse=False):
  ...
  for index in indices:#不断以新的last作为下一段的输出。
