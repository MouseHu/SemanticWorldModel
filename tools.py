    ...
    while True:
        ...
        for index in random.choice(len(keys), rescan):
            ...
            if length:
                ...
                if available < 1:
                    # yield episode
                    # print(f'Skipped short episode of length {available} {length}.')
                    continue
                if balance:
                ...
                # print("Sample episode length: ", total, "Index:", index)
            else:
                # print("Sample episode length: ", len(next(iter(episode.values()))))
                pass
            total_count += 1
            if episode['reward'][-1] > 0:
                goal_count += 1
                # print("reward", episode['reward'][-1])
            # print("Goal Sample rate:", goal_count / total_count)
            yield episode
...
def compute_cpc_loss(pred, features, config):
    dtype = {
        ...
        32:tf.float32,
        16:tf.float16,
    }.get(config.precision,tf.float32)
    if config.cpc_contrast == 'batch':
        ta = tf.TensorArray(dtype, 0, True, element_shape=[None, None])
        ...
        # print("positive")
        # print(positive.shape,ta.stack().shape)
        # tf.print("positive",positive)
        # tf.print(ta.stack())
        negative = tf.reduce_logsumexp(ta.stack(), 0)
        # print("negative")
        # print(negative.shape)
        # tf.print("negative",negative.shape,negative)
        ...
        assert config.cpc_batch_amount <= config.batch_size
        assert config.cpc_time_amount <= config.batch_length
        ...
        def compute_negatives(index, ta):
            ...
            time_shift = tf.compat.v1.math.mod(index, config.cpc_time_amount)
