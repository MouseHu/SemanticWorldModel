...
from gridworld.collect_data import model_to_onehot
import cv2
...
class GridWorld:
  ...
  def step(self, action):
    ...
    obs = {}
    obs['image'] = time_step
    obs['desc'] = self._desc_onehot
    return obs, reward, done, info

  def reset(self):
    if self._env.state.current_steps<70 and not self._env.state.done:
      print("odd current steps")
      print(self._env.state.current_steps)
    obs = {}
    time_step = self._env.reset()
    obs['image'] = time_step
    obs['desc'] = self._desc_onehot
...
class Collect:
  ...
  def step(self, action):
    ...
    #done: bool
    #info: dict {'discount'}
    #obs: dict {'orientations', 'height', 'velocity', 'image'(uint8)}
    ...
    if done:
      #print('episode length %d'% len(self._episode))
...
class OneHotAction:
  ...
  def step(self, action):
    #convert one-hot to index, but WHY?
