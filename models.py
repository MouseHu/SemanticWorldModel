...
class RSSM(tools.Module):
  ...
  def __init__(self, stoch=30, deter=200, hidden=200, act=tf.nn.elu, desc_len = 17):
    super().__init__()
  ...
  def initial(self, batch_size):#初始化输出状态
...
class ConvEncoder(tools.Module):
  ...
  def __call__(self, obs):
    ...
    if self._desc_len:
      desc = obs['desc']
    #print(obs['image'].shape)(25,50,64,64,3)
    ...
    if self._desc_len:
      #(-1,4,4,256)
      x_flatten = tf.reshape(x, (-1, 4 * 32 * self._depth))
