...
class RSSM(tools.Module):
    ...
    def obs_step(self, prev_state, prev_action, embed, desc):
        ...
        #:param :desc:(batch,dim)
        #:return :post: q(s|s,o,a)(batch,dim)
        #NOTE:ADDDESC
...
class ConvEncoder(tools.Module):
    ...
    def __call__(self, obs):
        ...
        shape = tf.concat([tf.shape(obs['image'])[:-3], [4 * 4 * 8 * self._depth]], 0)
        # x = self.get('h4', tfkl.Conv2D, 4 * self._depth, 4, **kwargs)(x)
        # shape = tf.concat([tf.shape(obs['image'])[:-3], [4 * 4 * 4 * self._depth]], 0)
        return tf.reshape(x, shape)


# class ConvDecoder(tools.Module):
#
#     def __init__(self, depth=32, act=tf.nn.relu, shape=(104, 104, 3)):
#         # NOTE:CHANEG:change shape
#         self._act = act
#         self._depth = depth
#         self._shape = shape
#
...
#
#         if self._shape[0] == 104:
#             x = self.get('h5', tfkl.Conv2DTranspose, 1 * self._depth, 6, **kwargs)(x)
#             x = self.get('h6', tfkl.Conv2DTranspose, self._shape[-1], 2, strides=2)(x)
#         else:
#             x = self.get('h5', tfkl.Conv2DTranspose, self._shape[-1], 2, strides=2)(x)
#
...
class ConvDecoder(tools.Module):
    ...
    def __call__(self, features):
        if self._shape[0] == 104:
            ...
            x = tf.reshape(x, [-1, 1, 1, 32 * self._depth])
            x = self.get('h2', tfkl.Conv2DTranspose, 4 * self._depth, 5, **kwargs)(x)
            x = self.get('h3', tfkl.Conv2DTranspose, 2 * self._depth, 4, **kwargs)(x)
            x = self.get('h4', tfkl.Conv2DTranspose, 1 * self._depth, 3, **kwargs)(x)
            x = self.get('h5', tfkl.Conv2DTranspose, 1 * self._depth, 4, **kwargs)(x)
            x = self.get('h6', tfkl.Conv2DTranspose, self._shape[-1], 2, strides=2)(x)
