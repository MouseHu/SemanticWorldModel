...
class Dreamer(tools.Module):
  ...
  def __init__(self, config, datadir, actspace, writer, desc_len = 17):
    #TODO:consider disc
    ...
    self._should_log = tools.Every(config.log_every)#每n step返回一次true
    self._last_log = None
    self._last_time = time.time()
    self._metrics = collections.defaultdict(tf.metrics.Mean)
    self._metrics['expl_amount']  # Create variable for checkpoint.
    self._float = prec.global_policy().compute_dtype
    self._strategy = tf.distribute.MirroredStrategy()#可被GPU共享的策略
  ...
  def __call__(self, obs, reset, state=None, training=True):
    ...
    #NOTE: Dreamer does not keep track of its own state.
    #NOTE: nothing todo with description since it's in obs.
    ...
    if self._should_train(step):
      ...
      with self._strategy.scope():
        for train_step in range(n):
          ...
          #so dataset is an iter of dicts
          #What does the first two dims in data mean？Episode, and length
  ...
  def _train(self, data, log_images):
    ...
    with tf.GradientTape() as model_tape:
      ...
      #(image_pred) tfp.distributions.Independent("IndependentNormal", 
      #batch_shape=[25, 50], event_shape=[64, 64, 3], dtype=float16)
    ...
    if tf.distribute.get_replica_context().replica_id_in_sync_group == 0:
      ...
      if tf.equal(log_images, True):
        self.image_summaries(data, embed, image_pred)

  def _build_model(self):
    #TODO:add desc input to RSSM and Encoder
    acts = dict(
...
def load_dataset(directory, config):#从data_dir中
  #NOTE: do not change since desc is in episode.
  ...
  dataset = dataset.prefetch(10)#预取缓冲区
  return dataset


def summarize_episode(episode, config, datadir, writer, prefix):
  #TODO: (optional)add desc
...
def main(config):
  ...
  #TODO:pass desc length to Dreamer
  agent = Dreamer(config, datadir, actspace, writer)
  if (config.logdir / 'variables.pkl').exists():
    print('Load checkpoint.')
    agent.load(config.logdir / 'variables.pkl')#possible to resume training
  state = None
...
if __name__ == '__main__':
  ...
  for key, value in config_gridworld().items():
